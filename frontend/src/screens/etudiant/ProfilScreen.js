...
const ProfilScreen = () => {
  ...
  const getBadgeColor = (badge) => {
    switch (badge) {
      case 'Expert':    return { bg: semantic.warningBg,  text: semantic.warning };
      case 'Confirmé':  return { bg: c.card,              text: c.primary };
      case 'Actif':     return { bg: semantic.successBg,  text: semantic.success };
      default:          return { bg: c.card,              text: c.subtext };
    }
  };
    ...
    >
      ...
      <View style={[styles.profileHeader, { backgroundColor: c.surface, borderBottomColor: c.border }]}>
        ...
        <View style={[styles.roleBadge, { backgroundColor: c.card }]}>
          ...
          <Text style={[styles.roleText, { color: c.primary }]}>Étudiant</Text>
        </View>
        ...
        <View style={styles.infoRow}>
          {user?.filiere && (
            <View style={[styles.infoChip, { backgroundColor: c.card }]}>
              <Ionicons name="school-outline" size={12} color={c.subtext} />
              <Text style={[styles.infoChipText, { color: c.subtext }]}>{user.filiere}</Text>
            </View>
          )}
          {user?.niveau && (
            <View style={[styles.infoChip, { backgroundColor: c.card }]}>
              <Ionicons name="ribbon-outline" size={12} color={c.subtext} />
              <Text style={[styles.infoChipText, { color: c.subtext }]}>{user.niveau}</Text>
            </View>
          )}
        </View>
      </View>
      ...
      {/* Réputation */}
      <View style={[styles.reputationCard, { backgroundColor: c.surface, borderColor: c.border }]}>
        <Text style={[styles.sectionTitle, { color: c.text }]}>Ma réputation</Text>
        <View style={styles.reputationRow}>
          <View style={styles.reputationItem}>
            <Ionicons name="star-outline" size={24} color={semantic.warning} />
            <Text style={[styles.reputationValue, { color: semantic.warning }]}>
              {reputation?.score || 0}
            </Text>
            <Text style={[styles.reputationLabel, { color: c.subtext }]}>Points</Text>
          </View>
          <View style={[styles.separator, { backgroundColor: c.border }]} />
          <View style={styles.reputationItem}>
            <Ionicons name="medal-outline" size={24} color={c.primary} />
            <Text style={[styles.reputationValue, { color: c.primary }]}>
              {reputation?.badge || 'Débutant'}
            </Text>
            <Text style={[styles.reputationLabel, { color: c.subtext }]}>Badge</Text>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  infoChip:         { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  infoChipText:     { fontSize: 12 },
  reputationCard:   { margin: 16, borderRadius: 14, borderWidth: 1, padding: 16 },
  sectionTitle:     { fontSize: 15, fontWeight: '700', marginBottom: 14 },
  reputationRow:    { flexDirection: 'row', alignItems: 'center' },
  reputationItem:   { flex: 1, alignItems: 'center', gap: 4 },
  reputationValue:  { fontSize: 20, fontWeight: '900' },
  reputationLabel:  { fontSize: 12 },
  separator:        { width: 1, height: 50 },
